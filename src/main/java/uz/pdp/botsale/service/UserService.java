...
public class UserService {
    ...
    public ApiResponse addUser(ReqSignUp request) {
        if (request.getId() == null) {
            ...
            return new ApiResponse("Foydalanuvchi muvoffaqiyatli ro'yxatga olindi", true, TrayIcon.MessageType.INFO);
        } else {
            ...
            if (optionalUser.isPresent()) {
                ...
                userRepository.save(user);
                return new ApiResponse("Foydalanuvchi malumotlari muvofaqqiyatli o'zgartirildi.", true, TrayIcon.MessageType.INFO);
            } else {
                return new ApiResponse("Bunday telefon raqamli foydalanuvchi mavjud", false, TrayIcon.MessageType.WARNING);
            }
        }
    }
    ...
    public ApiResponseModel getDocumentation(UUID id, Timestamp startDate,Timestamp entDate) {
        ApiResponseModel response = new ApiResponseModel();
        try {
            response.setMessage("Ok");
            response.setSuccess(true);
            User user = userRepository.findAllByCreatedAtAAndUpdatedAt(id, startDate, entDate).get();
